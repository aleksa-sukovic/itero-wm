...
const CONF_DIR: string = GLib.get_user_config_dir() + '/itero-wm';
...
export class Config {
        ...
        try {
            ...
            if (!conf.query_exists(null)) {
                ...
                if (!dir.query_exists(null) && !dir.make_directory(null)) {
                    return { tag: 1, why: 'failed to create itero-wm config directory' };
                }

                const example = new Config();
                example.float.push({ class: 'itero-wm-example', title: 'itero-wm-example' });
    ...
    private static read(): Result<string> {
        try {
            ...
            return { tag: 1, why: `failed to read itero-wm config: ${why}` };
        }
    }
