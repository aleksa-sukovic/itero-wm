...
function getSettings(schema: string) {
    let extensionPath = getExtensionPath('itero-wm@itero');
...
function launch_color_dialog() {
    let itero_wm_settings = getSettings('org.gnome.shell.extensions.itero-wm');
    ...
    if (rgba.parse(itero_wm_settings.get_string('hint-color-rgba'))) {
    ...
    if (response === Gtk.ResponseType.CANCEL) {
        ...
        itero_wm_settings.set_string('hint-color-rgba', color_dialog.get_rgba().to_string());
        Gio.Settings.sync();
        color_dialog.destroy();
    }
}
