...
Note that because the logs are from GNOME Shell, there will be messages from all installed extensions, and GNOME Shell itself. Itero WM is fairly chatty though, so the majority of the logs should be from Itero WM. Itero WM logs are usually prepended with `itero-wm:`, but sometimes GNOME has internal errors and warnings surrounding those logs that could be useful for pointing to an issue that we can resolve in Itero WM.
...
- [ ] Disabling window titles using global (Itero WM) option works for Shell Shortcuts, LibreOffice, etc.
